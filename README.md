...
- 竞技风 3D 场景重构：`EnvironmentKit` 接管核心战区，外围仍保持轻量填充几何体
- 玩家模型重构：`CharacterKit` 提供更清晰的人形轮廓、装备层次和 CT/T 克制识别点
- 渲染统一：`renderer.js` 维护战术灯光配置、功能性点光和中等强度后处理护栏
- 资产运行时：`RuntimeAssets` + kit 层负责后续高质量资产接入，同时保留缺失资产时的 graceful fallback
