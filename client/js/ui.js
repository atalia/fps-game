...
class UIManager {
  ...
  updatePlayerList(players) {
    ...
    players.forEach((player) => {
      ...
      div.innerHTML = `
        <span class="name">${team ? `<span style="color:${team.color}; font-weight:700;">[${team.label}]</span> ` : ""}${this.escapeHtml(name)}${player.is_bot ? " 🤖" : ""}</span>
  ...
  updateScoreboard(players, teams = null) {
    ...
    this.updateScoreboardTeamSummary(teams);
    ...
    sorted.forEach((player, index) => {
      ...
      tr.innerHTML = `
        ...
        <td style="color:${team?.color || "#aaa"}">${team?.label || "-"}</td>
  ...
  updateScoreboardTeamSummary(teams) {
    ...
    if (!teams || teams.length === 0) {
      this.elements.scoreboardTeamSummary.textContent = "";
      return;
    }

    this.elements.scoreboardTeamSummary.innerHTML = teams
      .map(
        (team) => `
      <span style="color:${team.color}; font-weight:800; letter-spacing:0.08em;">
        ${this.escapeHtml(team.short_name || team.id.toUpperCase())} ${team.score || 0}
      </span>
    `,
      )
      .join('<span style="color:#777;">&nbsp;|&nbsp;</span>');
  }
