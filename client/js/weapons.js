...
class WeaponSystem {
  ...
  // 获取所有武器
  getAllWeapons(teamId = null) {
    if (!teamId) {
      return Object.values(this.weapons);
    }

    const normalizedTeam = String(teamId || "")
      .trim()
      .toLowerCase();
    return Object.values(this.weapons).filter((weapon) => {
      if (weapon.legacy) return false;
      return !weapon.team || weapon.team === normalizedTeam;
    });
  }
  ...
  getUnlockedWeapons(level) {
    return Object.values(this.weapons).filter(
      (w) => !w.unlockLevel || w.unlockLevel <= level,
    );
  }

  getWeaponsForTeam(teamId) {
    return this.getAllWeapons(teamId);
  }
  ...
  calculateDamage(weapon, distance, headshot = false) {
    ...
    if (distance > weapon.range * 0.5) {
      const falloff =
        1 - (distance - weapon.range * 0.5) / (weapon.range * 0.5);
...
class WeaponShop {
                        ...
                        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px;">
                            ${typeWeapons
                              .map((w) => {
                                const locked =
                                  w.unlockLevel &&
                                  w.unlockLevel > this.playerLevel;
                                return `
  ...
  bindEvents() {
    this.element
      .querySelectorAll('.weapon-card:not([style*="rgba(50, 50, 50, 0.5)"])')
      .forEach((card) => {
